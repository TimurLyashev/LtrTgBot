...
auto findCity(std::vector<cityType> &cityVector,
              std::string cityNameFromMessage,
              std::string cityCountryFromMessage) {
  return std::find_if(cityVector.begin(), cityVector.end(),
                      [&](const auto &gr) {
                        return ((gr.name == cityNameFromMessage) &&
                                (gr.country == cityCountryFromMessage));
                      });
}

auto findUser(std::vector<userType> &userDB, int32_t userID) {
  return std::find_if(userDB.begin(), userDB.end(),
                      [&](const auto &gr) { return gr.id == userID; });
}
...
int main() {
  ...
  for (const auto &row :
       db(select(all_of(usersTab)).from(usersTab).unconditionally())) {
    newUser.id = row.ID;
    ...
    if (findUser(userDB, message->from->id) == userDB.end()) {
      ...
      if ((user->city == "placeholder") && (user->country == "placeholder")) {
        ...
        bot.getApi().sendMessage(
            ...
            "Введи имя города и страну. Пример: /setcity Moscow");
        bot.getApi().sendMessage(message->chat->id, "/setcountry RU");
      } else if ((user->city == "placeholder") &&
                 (user->country != "placeholder")) {
        bot.getApi().sendMessage(message->chat->id, "Привет!");
        bot.getApi().sendMessage(message->chat->id,
                                 "Введи имя города. Пример: /setcity Moscow");
        bot.getApi().sendMessage(message->chat->id,
                                 "Выбранная страна: " + user->country);
      } else if ((user->city != "placeholder") &&
                 (user->country == "placeholder")) {
        bot.getApi().sendMessage(message->chat->id, "Привет!");
        bot.getApi().sendMessage(message->chat->id,
                                 "Введи код страны. Пример: /setcountry RU");
        bot.getApi().sendMessage(message->chat->id,
                                 "Выбранный город: " + user->city);
      } else {
        bot.getApi().sendMessage(message->chat->id, "Привет!");
        bot.getApi().sendMessage(message->chat->id,
                                 "Выбранный город: " + user->city);
  ...
  bot.getEvents().onCommand("setcity", [&bot, &userDB, &newUser, &db,
                                        &usersTab](
    ...
    if (setcity.size() < 9) {
      ...
      if (findUser(userDB, message->from->id) == userDB.end()) {
        ...
        userDB.push_back(newUser);
        db(insert_into(usersTab).set(
            usersTab.ID = newUser.id, usersTab.CITY = newUser.city,
            usersTab.COUNTRY = newUser.country, usersTab.LANG = newUser.lang));
        bot.getApi().sendMessage(message->chat->id, "Выбран город: " + setcity);
      } else {
        user->city = setcity;
        db(update(usersTab)
               .set(usersTab.CITY = setcity)
               .where(usersTab.ID.in(user->id)));
        bot.getApi().sendMessage(message->chat->id, "Выбран город: " + setcity);
      }
    }
  });
  ...
  bot.getEvents().onCommand(
      "setcountry",
      [&bot, &userDB, &newUser, &db, &usersTab](TgBot::Message::Ptr message) {
        ...
        if (setcountry.size() < 12) {
          ...
          if (findUser(userDB, message->from->id) == userDB.end()) {
            ...
            userDB.push_back(newUser);
            db(insert_into(usersTab).set(usersTab.ID = newUser.id,
                                         usersTab.CITY = newUser.city,
                                         usersTab.COUNTRY = newUser.country,
                                         usersTab.LANG = newUser.lang));
            bot.getApi().sendMessage(message->chat->id,
                                     "Выбрана страна: " + setcountry);
          } else {
            user->country = setcountry;
            db(update(usersTab)
                   .set(usersTab.COUNTRY = setcountry)
                   .where(usersTab.ID.in(user->id)));
            bot.getApi().sendMessage(message->chat->id,
                                     "Выбрана страна: " + setcountry);
          }
  ...
  bot.getEvents().onNonCommandMessage([&bot, &weatherToken, &cityVector,
                                       &userDB](TgBot::Message::Ptr message) {
    printf("User wrote %s\n", message->text.c_str());
    ...
    if (findUser(userDB, message->from->id) == userDB.end()) {
      bot.getApi().sendMessage(
          ...
          "Введи имя города и страну. Пример: /setcity Moscow");
      bot.getApi().sendMessage(message->chat->id, "/setcountry RU");
    } else if (findCity(cityVector, user->city, user->country) ==
               cityVector.end()) {
      bot.getApi().sendMessage(message->chat->id,
                               "Сочетание города " + user->city + " и страны " +
                                   user->country + " не найдено.");
      bot.getApi().sendMessage(
          message->chat->id,
          "Введи новое имя города и страну. Пример: /setcity Moscow");
      bot.getApi().sendMessage(message->chat->id, "/setcountry RU");
    } else {
      CURL *curl;
      curl = curl_easy_init();
      ...
      std::string weatherUrl =
          "https://api.openweathermap.org/data/2.5/find?q=" + user->city + "," +
          user->country + "&units=metric&lang=ru&appid=" + weatherToken;
      CURLcode res;
      if (curl) {
      ...
      json weatherResponceJson =
          nlohmann::json::parse(weatherResponceStringBuffer);

      float ftemp = weatherResponceJson["list"][0]["main"]["temp"];
      std::string strtemp = std::to_string(ftemp);
      std::string description =
          weatherResponceJson["list"][0]["weather"][0]["description"];

      if (message->from->username == "egorpugin") {
        bot.getApi().sendMessage(
            message->chat->id, "В МУРМАНСКЕ (" + user->city + ") " +
                                   description +
                                   ", НО НЕ ЗАБЫВАЙ НАДЕВАТЬ ВАЛЕНКИ И УШАНКУ");
        bot.getApi().sendMessage(message->chat->id, strtemp);

      } else {
        bot.getApi().sendMessage(message->chat->id, "Погода в " + user->city +
                                                        " " + user->country);
